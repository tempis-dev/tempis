...
export class TempisTimeline {
    ...
    public focus(options?: FocusOptions): void {
        // If we are about to focus on something then we should clear any actove tooltip.
        this._tooltipView.clear();

        // The focus controller will handle the focusing for us, just pass the options.
    ...
    private _createCanvasEventHandlers() {
        ...
        this._eventHandlers.pointerup = (event) => {
            ...
            // Only process primary button (0 = left-click / touch) to avoid treating
            // right-click pointerup as a click on desktop.
            if (Math.sqrt(dx * dx + dy * dy) < dragPixelThreshold && event.button === 0) {
        ...
        this._eventHandlers.contextmenu = (evt) => {
            ...
            if (clickedItem) {
                // Suppress the default context handler.
                evt.preventDefault();

                // Disarm the pointer so the subsequent pointerup (from a touch long-press) is not processed as a click.
                // Without this, long-press on touch would fire both onItemContextClick and onItemClick for the same interaction.
                isPointerDown = false;

                // Also reset panning state so the scrollbar visibility doesn't get stuck.
                this._dataView.setPanning(false);
